...
private const val TAG = "PlaceDetails"
    ...
    if (searchText.isEmpty()) {
      ...
      try {
        ...
        searchResults = response.places.mapNotNull { place ->
          ...
          if (placeId != null && name != null && latLng != null) {
            PlaceDetails(
              placeId = placeId,
              name = name,
              address = address,
        ...
        Card(modifier = Modifier
          ...
          SearchResultsList(searchResults, focusedPlace) { place, doubleClick ->
            if (doubleClick) {
              selectedPlace = place
              scope.launch {
                cameraPositionState.animate(CameraUpdateFactory.newLatLngZoom(place.location, 15f))
              }
            } else {
...
private fun PlaceDetails(focusedPlace: PlaceDetails) {
  Column(
    ...
    Text(text = focusedPlace.address ?: "No address")
  }
}
...
private fun SearchResultsList(
  ...
  LazyColumn {
    items(searchResults) { place ->
      ...
      val modifier = if (place == focusedPlace) {
